...
class UserModelTestCase(unittest.TestCase):
    ...
    def test_valid_reset_token(self):
        ...
        self.assertTrue(User.reset_password(token, '123'))
    ...
    def test_invalid_reset_token(self):
        ...
        db.session.commit()
        token = u.generate_reset_token()
        self.assertFalse(User.reset_password(token + 'a', '1234'))
        self.assertTrue(u.verify_password('123456'))

    def test_valid_email_change_token(self):
        u = User(email='john@example.com', password='123456')
        db.session.add(u)
        db.session.commit()
    ...
    def test_invalid_email_change_token(self):
        u1 = User(email='john@example.com', password='123456')
        u2 = User(email='susan@example.org', password='12345')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
    ...
    def test_moderator_role(self):
        ...
        u = User(email='john@example.com', password='123456', role=r)
        self.assertTrue(u.can(Permission.FOLLOW))
        self.assertTrue(u.can(Permission.COMMENT))
        self.assertTrue(u.can(Permission.WRITE))
    ...
    def test_follows(self):
        u1 = User(email='john@example.com', password='123456')
        u2 = User(email='susan@exmaple.org', password='12345')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
    ...
    def test_to_json(self):
        ...
        with self.app.test_request_context('/'):
            json_user = u.to_json()
            expected_keys = ['url', 'username', 'member_since', 'last_seen', 'posts_url', 'followed_posts_url',
                             'post_count']
